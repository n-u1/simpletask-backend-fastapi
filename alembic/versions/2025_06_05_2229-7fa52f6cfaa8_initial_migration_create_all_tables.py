...
Revision ID: 7fa52f6cfaa8
Revises:
Create Date: 2025-06-05 22:29:53.950614+09:00
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("locked_until", sa.DateTime(), nullable=True, comment="アカウントロック解除日時"),
        sa.Column("id", sa.UUID(), nullable=False, comment="プライマリキー（UUID）"),
        sa.Column("created_at", sa.DateTime(), nullable=False, comment="作成日時（UTC）"),
        sa.Column("updated_at", sa.DateTime(), nullable=False, comment="更新日時（UTC）"),
    ...
    op.create_table(
        ...
        sa.Column(
            "priority", sa.String(length=10), nullable=False, comment="タスク優先度（low, medium, high, urgent）"
        ),
        sa.Column("due_date", sa.DateTime(), nullable=True, comment="タスクの期限日時（UTC）"),
        sa.Column("completed_at", sa.DateTime(), nullable=True, comment="タスク完了日時（UTC）"),
