...
async def get_tags(
    *,
    ...
    try:
        ...
        tag_list_response = await tag_service.get_tags(
            db,
            current_user.id,
            page=page,
            per_page=per_page,
...
async def create_tag(
    *, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user), tag_in: TagCreate
    ...
    try:
        # サービス層でタグ作成
        tag_response = await tag_service.create_tag(db, tag_in, current_user.id)
...
async def get_tag(
    *,
    ...
    try:
        # サービス層からPydanticレスポンスモデルを取得
        tag_response = await tag_service.get_tag(db, tag_id, current_user.id)
        if not tag_response:
...
async def update_tag(
    *,
    ...
    try:
        # サービス層でタグ更新
        tag_response = await tag_service.update_tag(db, tag_id, tag_in, current_user.id)
